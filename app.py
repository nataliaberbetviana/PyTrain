...
with aba4:
    with st.expander("✨ Cadastrar Novo Exercício"):
        ...
        n_serie = st.selectbox("Série", ["A", "B", "C"])
        if st.button("Salvar no Catálogo"):
            supabase.table("exercicios").insert(
                {"nome": n_nome, "serie_tipo": n_serie, "peso_kg": 0, "series": 3, "repeticoes": 12}).execute()
            st.success("Adicionado!")

    if st.button("📦 Gerar Backup CSV"):
        # Lógica de download
        st.write("Funcionalidade em nuvem: use o botão de exportar do Supabase ou peça para gerar aqui.")