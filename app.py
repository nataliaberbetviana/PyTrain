...
def get_supabase() -> Client:
    if not SUPABASE_URL or not SUPABASE_KEY:
        st.error("Variáveis de ambiente SUPABASE_URL e SUPABASE_KEY não encontradas.")
        st.stop()
...
DEFAULTS: dict = {
    ...
    "inicio_timer":          0.0,
    "timer_descanso":        0,
    "timer_descanso_inicio": 0.0,
    "timer_descanso_ativo":  False,
    "cardio_ativo":          False,
    "params_cardio":         None,
    "dist_real":             0.0,
    "t_cardio_start":        0.0,
...
def tela_completar_perfil():
    ...
    with st.form("form_perfil"):
        ...
        estado   = st.selectbox("Estado", ESTADOS, index=7)
...
# Link de ativação de conta via email
...
try:
    ...
    r = supabase.table("historico_treinos")\
        .select("data_execucao")\
        .eq("user_id", uid())\
        .gte("data_execucao", ini_mes.isoformat())\
        .execute()
...
if treinos_mes == 0:
    ...
    msg_treinos = f"{treinos_mes} treino(s) este mês. Continue assim! 🔥"
elif treinos_mes < 10:
    msg_treinos = f"{treinos_mes} treinos este mês. Você está em chamas! 🚀"
else:
    msg_treinos = f"{treinos_mes} treinos este mês. Lendária! 🏆"
...
# ─────────────────────────────────────────────────────────
# Helpers de estilo reutilizáveis (HTML inline)
# ─────────────────────────────────────────────────────────
...
_VAL_BOX = "flex:1;text-align:center;padding:4px 0"
_ROW     = "display:flex;align-items:center;gap:10px;margin:6px 0"
...
if _aba_atual == "home":
    ...
    try:
        _res_all = supabase.table("historico_treinos")\
            .select("data_execucao").eq("user_id", uid()).execute()
        ...
        _n_hoje  = len(_df_all[_df_all["data_execucao"] >= _hoje_dt]) if not _df_all.empty else 0
        _ult_data = (
            _df_all["data_execucao"].max().astimezone(fuso).strftime("%d/%m %H:%M")
            if not _df_all.empty else ""
        )
    except Exception:
        _streak = 0; _n_hoje = 0; _ult_data = ""

    try:
        _ini_sem = (hoje_agora - timedelta(days=hoje_agora.weekday())).replace(
            hour=0, minute=0, second=0, microsecond=0)
        _res_sem = supabase.table("historico_treinos")\
            .select("data_execucao").eq("user_id", uid())\
            .gte("data_execucao", _ini_sem.isoformat()).execute()
        _dias_sem = len(set(
            pd.to_datetime(r["data_execucao"]).date() for r in _res_sem.data
        )) if _res_sem.data else 0
    except Exception:
        _dias_sem = 0
...
<div style="background:#13132a;border-radius:14px;padding:14px;margin-bottom:10px">
  ...
  <div style="display:flex;gap:8px">
    <div style="flex:1;background:#0f0f1a;border-radius:10px;padding:10px;text-align:center">
      ...
      <div style="font-size:1.8rem;font-weight:900;color:#a78bfa">{treinos_mes}</div>
    </div>
    <div style="flex:1;background:#0f0f1a;border-radius:10px;padding:10px;text-align:center">
...
<div style="background:#13132a;border-radius:14px;padding:14px;margin-bottom:10px">
  ...
  <div style="height:8px;background:#1a1a3e;border-radius:4px">
    <div style="height:8px;background:{'#4ade80' if _pct_meta >= 100 else '#7c3aed'};border-radius:4px;width:{_pct_meta}%"></div>
  </div>
...
# ═══════════════════════════════════════════════════════
    ...
    if modo_treino == "Treino Livre":
        ...
        # Deletar item via query param (evita botão dentro de loop)
        _del_livre = st.query_params.get("del_livre", "")
        if _del_livre != "":
            try:
        ...
        if st.session_state.treino_livre_exs:
            st.caption(f"{len(st.session_state.treino_livre_exs)} exercício(s)")
            for i, ex in enumerate(st.session_state.treino_livre_exs):
                nota_txt = (
                    f"<div style='font-size:0.82rem;color:#c8cdd5;margin-top:2px'>{ex['nota']}</div>"
                    if ex.get("nota") else ""
                )
                st.markdown(f"""
<div style="background:#1a237e22;border:1px solid #3949ab55;border-radius:10px;
     padding:10px 14px;margin-bottom:6px;position:relative">
  <a href="?del_livre={i}" style="position:absolute;top:8px;right:10px;
     color:#9ca3b0;font-size:1.1rem;text-decoration:none">✕</a>
  <div style="font-size:0.9rem;font-weight:700;color:#90caf9;padding-right:24px">
    {i+1}. {ex['nome']}</div>
  ...
  {nota_txt}
    ...
    # ── Treino por Série ──────────────────────────────────────────────────────
        ...
        if not st.session_state.treino_ativo:
            ...
            # Deletar exercício (pendente via session_state)
            if st.session_state.del_ex_pending:
                supabase.table("exercicios")\
                    .delete().eq("id", st.session_state.del_ex_pending).execute()
                st.session_state.del_ex_pending = None
                st.rerun()

            pode_iniciar = bool(exs.data)

            if exs.data:
                n_exs = len(exs.data)
                if st.button(
                    f"🚀 Iniciar Série {serie} — {n_exs} exercícios",
                ...
                with st.expander(f"👁 Ver {n_exs} exercícios da Série {serie}", expanded=False):
                    for i, ex in enumerate(exs.data, 1):
                        ult_det, ult_data_ex = _ultima_carga(ex["id"])
                        ult_html = (
                            f"<div style='font-size:0.72rem;color:#a78bfa;margin-top:3px'>"
                            f"📌 {ult_data_ex}: {ult_det}</div>"
                            if ult_det else ""
                        )
...
<div style="background:#1a237e22;border:1px solid #3949ab55;border-radius:10px;
  ...
  <div style="font-size:0.85rem;color:#c8cdd5;margin-top:1px">
    {ex['series']}×{ex['repeticoes']} · {ex['peso_kg']} kg</div>
  {ult_html}
</div>""", unsafe_allow_html=True)
                        with c_del:
                            st.write("")
                ...
                st.caption(f"SÉRIE {serie}  ·  VAZIA")

            # Adicionar exercício
            with st.expander(f"＋ Adicionar exercício — Série {serie}", expanded=not pode_iniciar):
                with st.form(f"form_add_{serie}_{st.session_state.n_adds}"):
            ...
            with st.expander("📋 Clonar esta série em outra"):
                ...
                dest   = st.selectbox(f"Clonar Série {serie} para →", outras, key="clone_dest")
                if st.button("Clonar", key="btn_clone"):
                    if exs.data:
                        for ex in exs.data:
                            dup = supabase.table("exercicios").select("id")\
                                .ilike("nome", ex["nome"])\
                                .eq("serie_tipo", dest).eq("user_id", uid()).execute()
                            if not dup.data:
                                supabase.table("exercicios").insert({
                                    "user_id":    uid(),
                                    "nome":       ex["nome"],
        ...
        # ── Execução do treino ────────────────────────────────────────────────
            ...
            _res_todos = supabase.table("exercicios")\
                .select("*")\
                .eq("serie_tipo", st.session_state.serie_atual)\
                .eq("user_id", uid()).order("id").execute()
            ...
            if not _res_todos.data:
                ...
                ordem    = st.session_state.ordem_exercicios or [e["id"] for e in _res_todos.data]
                ...
                if idx >= total:
                    try:
                        hist_all = supabase.table("historico_treinos")\
                            .select("data_execucao").eq("user_id", uid()).execute()
                ...
                # Valores editáveis por exercício
                ...
                ult_det, ult_data_ex = _ultima_carga(ex["id"])
                ult_txt = f"📌 {ult_data_ex}: {ult_det}" if ult_det else ""

                # Processar ações via query params
                _acao = st.query_params.get("acao", "")

                _acao_map = {
                    "mp": lambda: setattr(st.session_state, pk, max(0, p - 1))  or None,
                    "pp": lambda: setattr(st.session_state, pk, p + 1) or None,
                    "ms": lambda: setattr(st.session_state, pk, max(1, s - 1)) or None,  # noqa
                    "ps": lambda: setattr(st.session_state, pk, s + 1) or None,
                    "mr": lambda: setattr(st.session_state, pk, max(1, r - 1)) or None,  # noqa
                    "pr": lambda: setattr(st.session_state, pk, r + 1) or None,
                }

                if _acao in ("mp", "pp"):
                    st.session_state[pk] = max(0, p - 1) if _acao == "mp" else p + 1
                    st.query_params.clear(); st.rerun()
                elif _acao in ("ms", "ps"):
                    st.session_state[sk] = max(1, s - 1) if _acao == "ms" else s + 1
                    st.query_params.clear(); st.rerun()
                elif _acao in ("mr", "pr"):
                    st.session_state[rk] = max(1, r - 1) if _acao == "mr" else r + 1
                    st.query_params.clear(); st.rerun()
                elif _acao in ("r30", "r60"):
                    dur = 30 if _acao == "r30" else 60
                    st.session_state.timer_descanso       = dur
                    ...
                    _nota   = st.session_state.get(f"nota_{idx}", "")
                    det     = f"{p}kg | {s}x{r} | {elapsed // 60}min"
                    if _nota:
                        det += f" | {_nota}"
                    if is_pr:
                        det += " | 🏆 PR"
...
<div style="background:#1a1a2e;border-radius:14px;padding:14px;margin-bottom:6px">
  ...
  <div style="{_ROW}">
    <a href="?acao=mp" style="{_BTN_INLINE}">−</a>
    <div style="{_VAL_BOX}">
    ...
    <a href="?acao=pp" style="{_BTN_INLINE}">+</a>
  </div>

  <div style="{_ROW}">
    <a href="?acao=ms" style="{_BTN_INLINE}">−</a>
    <div style="{_VAL_BOX}">
    ...
    <a href="?acao=ps" style="{_BTN_INLINE}">+</a>
  </div>

  <div style="{_ROW}">
    <a href="?acao=mr" style="{_BTN_INLINE}">−</a>
    <div style="{_VAL_BOX}">
    ...
    <a href="?acao=pr" style="{_BTN_INLINE}">+</a>
  </div>
</div>""", unsafe_allow_html=True)

                st.text_input(
                    "📝 Nota", placeholder="Observação...",
                    key=f"nota_{idx}", label_visibility="collapsed",
                )
                ...
                if st.session_state.timer_descanso_ativo:
                    ...
                    timer_bar = (
                        f'<div style="height:4px;background:#333;border-radius:2px;margin:6px 0">'
                        f'<div style="height:4px;background:{timer_color};border-radius:2px;'
                        f'width:{int(pct_rest*100)}%"></div></div>'
                        f'<div style="text-align:center;font-size:1.5rem;font-weight:bold;'
                        f'color:{timer_color};margin-bottom:6px">{timer_txt}</div>'
                    )
...
<div style="margin-top:8px">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
    ...
    <div>
      <a href="?acao=r30" style="{_circle}">30s</a>
      <a href="?acao=r60" style="{_circle}">60s</a>
    </div>
  </div>
  <div style="display:flex;align-items:center">
    <a href="?acao=pular" style="{_btn_pular}">⏭ Pular</a>
    <a href="?acao=proximo" style="{_btn_prox}">✅ Próximo →</a>
  </div>
</div>""", unsafe_allow_html=True)
    ...
    if not st.session_state.cardio_ativo:
        ...
        try:
            ini_sem = (hoje_agora - timedelta(days=hoje_agora.weekday())).replace(
                hour=0, minute=0, second=0, microsecond=0)
            res_sem = supabase.table("historico_treinos")\
                .select("detalhes").eq("user_id", uid())\
                .gte("data_execucao", ini_sem.isoformat()).execute()
        ...
        if st.session_state._encerrar_cardio:
...
# ═══════════════════════════════════════════════════════
    ...
    try:
        ...
        if not res_h.data:
            ...
            if so_hoje:
                ini_dt = datetime.combine(hoje_agora.date(), datetime.min.time()).replace(tzinfo=fuso)
                fim_dt = datetime.combine(hoje_agora.date(), datetime.max.time()).replace(tzinfo=fuso)
                ...
                if modo_filtro == "Mês":
                    ...
                    titulo_resumo = "RESUMO — " + (
                        ini_sel.strftime("%d/%m/%Y") if ini_sel == fim_sel
  ...
  <div style="background:#1a1a2e;border-radius:10px;padding:10px;text-align:center">
    ...
    <div style="font-size:1.4rem;font-weight:bold">{round(km_f, 1)} km</div>
            ...
            if df_f.empty:
                ...
                st.dataframe(
                    df_show[["Data", "exercicios.nome", "detalhes"]].rename(
                ...
                st.download_button(
                    "📥 Exportar CSV", csv,
                    file_name="pytrain_historico.csv",
            ...
            if st.session_state.confirmar_historico:
...
# ═══════════════════════════════════════════════════════
    ...
    if _sub_evolucao == "🏋️ Progressão":
        ...
        try:
            ...
            res_peso = supabase.table("peso_corporal")\
                .select("*").eq("user_id", uid()).order("data").execute()
        ...
        try:
            with st.form("form_medidas"):
                ...
                vals_med = {
                    campo: cols_med[i % 3].number_input(label, value=0.0, step=0.5, min_value=0.0)
            ...
            res_med = supabase.table("medidas_corporais")\
                .select("*").eq("user_id", uid()).order("data").execute()
            ...
            if res_med.data:
                ...
                st.dataframe(
                    df_tab_m.rename(columns=rename_m).iloc[::-1],
                    use_container_width=True, hide_index=True,
                )
            else:
...
elif _aba_atual == "conquistas":
    st.info("🏆 " + _frase("conquistas"))

    try:
        res_conq = supabase.table("conquistas")\
            .select("conquista_id,desbloqueada_em").eq("user_id", uid()).execute()
        desbloqueadas = {r["conquista_id"]: r["desbloqueada_em"] for r in (res_conq.data or [])}

        res_all_h = supabase.table("historico_treinos")\
            .select("data_execucao").eq("user_id", uid()).execute()
...
<div style="background:#13131f;border:1px solid #2d2d45;border-radius:8px;
  ...
  <div style="color:#9ca3b0;font-size:0.88rem">{c['desc']}</div>
        ...
        if meta_atual:
            ...
            pct_tr = min(int(tr_mes  / max(int(meta_atual.get("treinos",      1)), 1) * 100), 100)
            pct_km = min(int(km_mes  / max(float(meta_atual.get("distancia_km", 1)), 0.01) * 100), 100)
            pct_mn = min(int(min_mes / max(int(meta_atual.get("tempo_min",    1)), 1) * 100), 100)
...
# ═══════════════════════════════════════════════════════
    ...
    try:
        rp = supabase.table("perfis").select("nome,telefone,cidade,estado")\
...
.perfil-label { color:#b0b8c8;font-size:0.82rem;text-transform:uppercase;letter-spacing:.05em;margin-bottom:2px; }
.perfil-valor { color:#e2e8f0;font-size:0.95rem;font-weight:500; }
.perfil-icon  { font-size:1.2rem;flex-shrink:0; }
</style>""", unsafe_allow_html=True)

    def card_perfil(icon: str, label: str, valor: str):
        st.markdown(f"""
  ...
  <div>
    ...
    with st.expander("✏️ Editar dados pessoais"):
        with st.form("form_dados"):
            ...
            idx_e     = ESTADOS.index(dp.get("estado", "ES")) if dp.get("estado", "ES") in ESTADOS else 7
            ed_estado = st.selectbox("Estado", ESTADOS, index=idx_e)
